...
function Assert-LeapInstallerArchitecture {
    ...
    if ($machine -eq 0x014C) {
        # The current official LEAP Desktop x64 download is a signed 32-bit
        # bootstrapper that deploys the x64 payload. Accept it only when the
        # vendor's x64 identity is explicit; reject generic x86 installers.
        ...
        if ($identity -match "(?i)(?:X64|64[- ]?bit)") {
            Write-Log "Verified signed LEAP x64 bootstrapper: PE bootstrap architecture is x86, but the vendor identity explicitly names the x64 payload ($Path)." "WARN"
            return
        }
    }

    if ($machine -ne 0x8664) {
        throw "LEAP installer is not an x64 payload or an explicitly x64-named vendor bootstrapper. PE machine value was 0x$('{0:X4}' -f $machine): $Path"
    }
