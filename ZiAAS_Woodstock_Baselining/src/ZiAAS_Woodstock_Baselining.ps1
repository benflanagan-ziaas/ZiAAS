...
function Import-ZiaasDeploymentConfig {
    ...
    $allowedNames = @(
        "WorkingRoot", "BrandPackPath", "InstallMode", "Simulation", "ForceCloseApps", "SkipOffice", "SkipAdobe",
        ...
        "NoComponentDownload", "UseCachedInstallers", "ManifestUrl", "ExpectedManifestHash",
        "DownloadRetryCount", "DownloadRetryDelaySeconds"
    )
...
function Get-ZiaasNextAction {
    ...
    if ($Status -eq "RebootRequired") {
        return "Restart the device, then confirm Office, Adobe, and LEAP open as expected."
    }
...
function Import-ZiaasResumeState {
    ...
    $Script:ResumeCompletedFiles = @($summary.Components | Where-Object { $_.Status -in @("Success", "RebootRequired") } | Select-Object -ExpandProperty File)
